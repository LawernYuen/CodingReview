package CodingDaily.LeCo;

public class IsInterleaveNo97 {
    public boolean isInterleave(String s1, String s2, String s3) {
//        if (s1.length() == 0 && s2.length() == 0 && s3.length() == 0)
//            return true;
//        if (s1.length() == 0){
//            if (s2.length() * s3.length() == 0 || s2.charAt(0) != s3.charAt(0))
//                return false;
//            else if (s2.charAt(0) == s3.charAt(0))
//                return isInterleave(s1, s2.substring(1), s3.substring(1));
//        }
//        if (s2.length() == 0){
//            if (s1.length() * s3.length() == 0 || s1.charAt(0) != s3.charAt(0))
//                return false;
//            else if (s1.charAt(0) == s3.charAt(0))
//                return isInterleave(s1.substring(1), s2, s3.substring(1));
//        }
//        if (s1.charAt(0) == s3.charAt(0) && s2.charAt(0) == s3.charAt(0))
//            return isInterleave(s1.substring(1), s2, s3.substring(1)) ||
//                    isInterleave(s1, s2.substring(1), s3.substring(1));
//        else if (s1.charAt(0) == s3.charAt(0))
//            return isInterleave(s1.substring(1), s2, s3.substring(1));
//        else if (s2.charAt(0) == s3.charAt(0))
//            return isInterleave(s1, s2.substring(1), s3.substring(1));
//        else
//            return false;
        if (s3.length() != s1.length()+s2.length())
            return false;
        boolean[][] dp = new boolean[s1.length()+1][s2.length()+1];
        dp[0][0] = true;
        for (int i = 1; i <= s1.length(); i++)
            dp[i][0] = dp[i-1][0] && (s1.charAt(i-1)==s3.charAt(i-1));
        for (int i = 1; i <= s2.length(); i++)
            dp[0][i] = dp[0][i-1] && (s2.charAt(i-1)==s3.charAt(i-1));
        for (int i = 1; i <= s1.length(); i++)
            for (int j = 1; j <= s2.length(); j++)
                dp[i][j] = (dp[i-1][j] && (s1.charAt(i-1)==s3.charAt(i-1+j))) ||
                        (dp[i][j-1] && (s2.charAt(j-1)==s3.charAt(j-1+i)));
        return dp[s1.length()][s2.length()];
    }

    public static void main(String[] args) {
        String s1 = "aabcc";
        String s2 = "dbbca";
        String s3 = "aadbbcbcac";
        IsInterleaveNo97 demo = new IsInterleaveNo97();
        boolean ans = demo.isInterleave(s1, s2, s3);
        System.out.println(ans);
    }
}
